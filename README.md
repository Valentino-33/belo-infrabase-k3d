...
| 4 | `wait-argocd-sync` | `bitnami/kubectl` | (a) Force-refresh ArgoCD app; (b) Espera `.status.sync.revision == commit-sha`; (c) Auto-detecta strategy + modo; (d) Espera `Rollout.spec.image-tag == <tag>` Y el estado correcto: `phase=Paused` (legacy) / RS-ready (enterprise) / `phase=Healthy` (Rolling). **Esta task garantiza que Stage 5 corre sobre la versión nueva** | **Results: `primary-env`, `strategy`** |
| 5 | `run-load-test` | `bitnami/kubectl` + `grafana/k6` | Step `detect-mode` introspecciona el Rollout; step `run-k6` corre k6 según strategy. **Modo legacy**: k6 valida (gated por flag `loadtest`). **Modo enterprise**: k6 genera tráfico para el `AnalysisRun` de Argo (corre siempre) | **Result: `outcome`** = `passed` \| `failed` |
| 6 | `promote-or-rollback` | `bitnami/kubectl` | Detecta modo por introspección.<br/>**Legacy**: patches directos — BG `status.pauseConditions=null`, Canary `status.promoteFull=true`, failed `spec.abort=true`.<br/>**Enterprise**: solo observa `phase=Healthy`/`Degraded` — Argo Rollouts ya decidió vía `AnalysisRun` | — |

> **Modelo enterprise** (`analysis.enabled: true` en `<app>/<env>/values.yaml`): el chart genera un `AnalysisTemplate` con queries Prometheus (success-rate ≥ 99%, latency-p95 < 1s) y configura el Rollout con `prePromotionAnalysis`+`postPromotionAnalysis` (BG) o un `analysis` en cada `setWeight` (Canary multi-step 5/25/50/100). Argo Rollouts decide promote/abort y hace **rollback automático** si las métricas fallan. El pipeline pasa de orquestador a observador. Ver [docs/deployment-strategies.md → dos modelos de promoción](docs/deployment-strategies.md#dos-modelos-de-promoción-legacy-y-enterprise).
